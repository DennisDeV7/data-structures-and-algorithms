...
const toLastNames = people => {
  let nameArray = people.map(person => (person.firstName + ' ' + person.lastName))
  return nameArray;
};
...
const addPurchases = (arr) => {
  let purchases = arr.reduce((total, currentItem) => {
    return (total + currentItem.purchasePrice);
  }, 0);
  return purchases;
};
...
const countNumberOfElements = (arr) => {
  let count = arr.reduce((total, currentItem) => {
    return total + 1;
  }, 0);
  return count;
};
...
const returnNames = (arr) => {
  let name = arr.reduce((starPeople, person) => {
    starPeople.push(person.name);
    return starPeople;
  }, [])
  return name;
};
...
const reversedString = (str) => {
  let wordArray = [...str].reduce((a, b) => [b, ...a], []);
  let newString = wordArray.reduce((a,b) => a + b);
  return newString;
};

reversedString('dennis');
